...
## Current: v`r ftolr::ft_data_ver()`

```{r}
backbone_n <-
  ftolr::ft_tree(branch_len="raw", backbone = TRUE, drop_og = TRUE) |>
  ape::Ntip()

full_tree_n <-
  ftolr::ft_tree(drop_og = TRUE) |>
  ape::Ntip() |>
  scales::number(big.mark = ",")
```

Built with data from GenBank release 251 (cutoff date 2022-08-15). Includes `r backbone_n` complete plastome sequences and `r full_tree_n` tips total (outgroups excluded unless othewise mentioned).
